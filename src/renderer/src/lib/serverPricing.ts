...
export function configuredCost(i: ConfiguredCostInput): ConfiguredCost {
  ...
  if (i.offsiteBackups && selectedBackups > 0) {
     ...
     * web panel does not: mPanel picks the first
     ...
     * subtracts inclusions. This matches mPanel, which sums the
 ...
 *
 * The rule is mPanel's, not a clamp. It
 * clears the previous value whenever a
 * different base size is chosen, and it
 * then resolves to the target plan's default - so
 * changing plan hands the customer the *target plan's* included transfer, while
 * keeping the same plan keeps whatever they had. The API agrees:
 * it resets additional transfer to zero on a
