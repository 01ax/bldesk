...
If your key is elsewhere, choose Browse… and select its existing private-key file. Any filename and accessible folder can be selected; a matching .pub file is not required. The exact filename appears in the selector, with the full path displayed below it under Key file. Cancelling leaves the association unchanged. BLDesk checks file metadata only; OpenSSH reads the key and handles passphrase prompts, or uses your SSH agent. BLDesk does not store passphrases. If the file is moved or renamed, browse to its new path.
