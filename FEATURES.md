...
- **Server Details parity with mPanel** (v1.0.27): a **Network** tab (interfaces, IPv6, port blocking, VPC membership, Edge DDoS status), a **Settings** tab covering the full mPanel settings suite, and a **Usage** tab with the mPanel-style metrics graphs (paged sample sets, independent per-metric scaling, live-telemetry fallback cards, day/week/month/year windows).
...
- **Make `Server.status` track power state**. Today it never turns `off`; the client infers power from sample staleness plus an `is_running` after each power action (`lib/powerState.ts`). The real fix is on the API side.
